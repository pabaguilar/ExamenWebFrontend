...
function Update(){
  ...
  try {
    const response = await axios.put(apiEndpoint.api+"eventos/" + data._id, updatedVersion, {headers: { "Content-Type": "application/json" },});
    ...
    <div className='min-h-screen bg-gray-100 text-black p-5 w-full sm:w-5/6 md:w-5/6 lg:w-4/6 mx-auto rounded-lg shadow-2xl bg-white'>
      ...
      <h2 className='pt-4 text-3xl font-bold mb-2' >Editar Evento</h2>
        <form onSubmit={handleEventSubmit} className='flex flex-col'>
          ...
          <button type='submit' className="bg-green-500 text-white mt-2 w-fit mx-auto px-4 py-2 rounded hover:bg-green-600 transition">Editar Evento</button>
        </form>
